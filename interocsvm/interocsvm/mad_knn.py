...
class MadKnn():
    ...
    The method estimates the fraction of outliers in the dataset based on
    the median absolute deviation (MAD) based on the KNN-average similarity.
    This method can be used with any python module for graph kernel
    estimation as long as the kernel have a fit_transform method returning a
    square numpy array.
    ...
    def fit(self, n = 7, plots = True, find_n = True):
        '''
        Find the nu value for the given dataset.
        The method can be called multiple times changing the parameters,
        without the need to recompute the similarity matrix.
        ...
        :type plots: bool
        :param find_n: Defines K of KNN as the value corresponding to 3% of
                       the data
        :type find_n: bool
        '''
        if find_n:
        ...
        self.nu = len(self.vals[mask])/len(self.vals)
        if self.nu == 0:
            self.nu = 1e-3
        return mask

def plot_madknn(ax, mask, vals):
    ax.clear()
    ax.plot(np.arange(len(vals[mask]), len(vals)), np.sort(vals[np.invert(mask)]), color = 'blue', label = 'Inlier')
    ax.plot(np.arange(len(vals[mask])), np.sort(vals[mask]), color = 'red', label = 'Outlier')
    ax.legend()
    ax.set_xlabel('Rank')
